...
class YachtOrdering extends React.Component {
  constructor(props) {
    super(props);
  ...
  */
  onClick(info) {
    ...
    if (filterArray.length > 0) {
      ...
      this.setState({
        search_term: this.props.t(`common:${filterArray[0].value}`),
      });
    } else {
      this.setState({
        search_term: "",
  ...
  */
  renderValues() {
    return this.state.values.map(function (value) {
      return <DropdownItem key={value.value}>{value.label}</DropdownItem>;
    });
  }
...
YachtOrdering.defaultProps = {
  values: [],
};

export default withTranslation()(YachtOrdering);
