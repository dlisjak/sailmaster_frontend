...
export default class CreditCardNumber extends React.Component {
  ...
  onInputBlur() {
    if (this.state.value.length !== 19) this.setState({ error: true });
  ...
  onInputChange(e) {
    if (
      ...
      (e.target.value.length === 0 &&
        e.target.value.length < this.state.value.length)
    ) {
      if (
        ...
        let _tmp = e.target.value[e.target.value.length - 1];
        e.target.value =
          e.target.value.substring(0, e.target.value.length - 1) + "-" + _tmp;
      }

      if (
        (e.target.value.length === 4 && this.state.value.length === 3) ||
        (e.target.value.length === 9 && this.state.value.length === 8) ||
        (e.target.value.length === 14 && this.state.value.length === 13)
      )
        e.target.value += "-";
      ...
      if (this.state.value.length === 19) this.setState({ error: false });
    }
  }
