...
export default class CreditCardExpDate extends React.Component {
  constructor(props) {
    super(props);
  ...
  onInputBlur() {
    if (this.state.value.length !== 5) this.setState({ error: true });
  ...
  onInputChange(e) {
    if (
      ...
      if (e.target.value.length === 2 && this.state.value.length === 1)
        e.target.value += "/";
      ...
      if (this.state.value.length === 5) this.setState({ error: false });
    }
  }
