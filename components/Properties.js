    ...
    {chunk(items, size).map((rows, rowIndex) => (
      <div className="row" key={rowIndex}>
        {range(size).map((index) => {
          ...
          if (!property) {
            return <div className="relative flex-grow max-w-full flex-1 px-[15px]" key={index} />;
          }
          return <div className="relative flex-grow max-w-full flex-1 px-[15px]" key={index}><Property {...property} /></div>;
        })}
      </div>
    ))}
  </div>
