...
const OffersPage = ({ search, results, destination, fallback, canonicalUrl }) => {
  ...
  const [filterValues, setFilterValues] = useState({
    destinations: [{ label: 'Slovenija', obj_type: 'country', value: '201' }],
  });
  ...
  return (
    <SWRConfig value={{ fallback }}>
      <Head>
        <title>{t('rental_meta_title_slovenija')}</title>
        <meta name="title" content={t('rental_meta_title_slovenija')} />
        <meta name="description" content={t('rental_meta_description')} />

        <meta property="og:type" content="website" />
        <meta property="og:url" content={canonicalUrl} />
        <meta property="og:title" content={t('rental_meta_title_slovenija')} />
        ...
        <meta property="twitter:title" content={t('rental_meta_title_slovenija')} />
...
export const getStaticProps = async ({ locale }) => {
  ...
  const search = '/najem-plovil?destinations=country-201-Slovenija';

  const { data } = await getSearchResultsFromApi(search);

  const najemPlovil = '/najem-plovil/slovenija';
