...
const OffersPage = ({ search, results, destination, fallback, canonicalUrl }) => {
  ...
  const [filterValues, setFilterValues] = useState({
    destinations: [{ label: 'Split regija', obj_type: 'region', value: '123' }],
  });
  ...
  return (
    <SWRConfig value={{ fallback }}>
      <Head>
        <title>{t('rental_meta_title_split')}</title>
        <meta name="title" content={t('rental_meta_title_split')} />
        <meta name="description" content={t('rental_meta_description')} />

        <meta property="og:type" content="website" />
        <meta property="og:url" content={canonicalUrl} />
        <meta property="og:title" content={t('rental_meta_title_split')} />
        ...
        <meta property="twitter:title" content={t('rental_meta_title_split')} />
...
export const getStaticProps = async ({ locale }) => {
  ...
  const search = '/najem-plovil?destinations=region-123-Split%20regija';

  const { data } = await getSearchResultsFromApi(search);

  const najemPlovil = '/najem-plovil/split';
