...
const OffersPage = ({ search, results, destination, fallback, canonicalUrl }) => {
  ...
  const [filterValues, setFilterValues] = useState({
    destinations: [{ label: 'Črna Gora', obj_type: 'country', value: '149' }],
  });
  ...
  return (
    <SWRConfig value={{ fallback }}>
      <Head>
        <title>{t('rental_meta_title_crna_gora')}</title>
        <meta name="title" content={t('rental_meta_title_crna_gora')} />
        <meta name="description" content={t('rental_meta_description')} />

        <meta property="og:type" content="website" />
        <meta property="og:url" content={canonicalUrl} />
        <meta property="og:title" content={t('rental_meta_title_crna_gora')} />
        ...
        <meta property="twitter:title" content={t('rental_meta_title_crna_gora')} />
...
export const getStaticProps = async ({ locale }) => {
  ...
  const search = '/najem-plovil?destinations=country-149-Črna%20Gora';

  const { data } = await getSearchResultsFromApi(search);

  const najemPlovil = '/najem-plovil/crna-gora';
